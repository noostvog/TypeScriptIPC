...
namespace Proplog_parse {
    export function parse(txt: any) {
        var tmp;
        /*
        tmp = parse_dimacs(txt);
        if (tmp[0] !== 0 && tmp.length > 1) {
            // possibly dimacs
            if (tmp[1].length > 0) return tmp;
        }*/
        // here we assume non-dimacs, parse as formula
        tmp = parse_formula(txt);
        return tmp;
    }
    ...
    */
    ...
    export function parse_tree(txt: any) {
        var tmp, i;
        let res: any;
    ...
    */
    ...
    /*
    export function parse_dimacs(txt: any): string[] {
...
*/
    ...
    */
    ...
    export function parse_formula(txt: any) {
        var tmp, exp, pos;
    ...
    function parse_expression_tree(txt: any, pos: any): any {
        var isneg, exp, op, tmp;
        pos = parse_skip(txt, pos);
        isneg = false;
        if (parse_isatpos(txt, pos, "-")) {
            isneg = true;
            pos++;
        }
        else if (parse_isatpos(txt, pos, "~")) {
            isneg = true;
            pos++;
        }
        else if (parse_isatpos(txt, pos, "!")) {
            isneg = true;
            pos++;
        } //<nathalie> own addition to work with existing translations
        ...
        while (true) {
            op = null;
            if (parse_isatpos(txt, pos, "&") === true) {
                op = "&";
                pos++;
            }
            else if (parse_isatpos(txt, pos, "and") === true) {
                op = "&";
                pos += 3;
            }
            else if (parse_isatpos(txt, pos, "+") === true) {
                op = "+";
                pos++;
            }
            else if (parse_isatpos(txt, pos, "xor") === true) {
                op = "+";
                pos += 3;
            }
            else if (parse_isatpos(txt, pos, "|") === true) {
            ...
            else if (parse_isatpos(txt, pos, "v") === true) {
                op = "V";
                pos++;
            }
            else if (parse_isatpos(txt, pos, "V") === true) {
            ...
            else if (parse_isatpos(txt, pos, "or") === true) {
                op = "V";
                pos += 3;
            }
            else if (parse_isatpos(txt, pos, "->") === true) {
    ...
    function parse_term_tree(txt: any, pos: any): any {
        ...
        while (true) {
            ...
            n = txt.charCodeAt(j);
            // numbers and letters
            if (j < txt.length && ((n >= 48 && n <= 57) || (n >= 65 && n <= 122))) {
                j++;
                continue;
            }
    ...
    function parse_isatpos(txt: any, pos: any, str: any) {
        if (str.length === 1) {
    ...
    function parse_skip(txt: any, pos: any) {
        var c;
    ...
    function parse_error(txt: any) {
        return ["error", txt];
    }

    function err_data(data: any) {
